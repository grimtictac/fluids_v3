...
void FluidSystem::SetupAddVolume ( Vector3DF min, Vector3DF max, float spacing, float offs )
{
	Vector3DF pos;
	int n, p;
...
//	Setup ();
	printf ( "VALIDATION:\n" );
	InsertParticles ();	printf ( "  Insert. OK\n" );	
	FindNbrsSlow ();	printf ( "  True Neighbors. OK\n" );	
	SaveResults ();		printf ( "  Save Results. OK\n" );	
	Run (0,0);			printf ( "  New Algorithm. OK\n" );
	...
	for (int n=0; n < NumPoints(); n++ ) {
		if ( *(mSaveCnt+n) != *(mNbrCnt+n) ) {
			*(mClr+n) = COLORA(1.0,0,0,0.9);
			printf ( "Error %d, correct: %d, cnt: %d\n", n, *(mSaveCnt+n), *(mNbrCnt+n) );
		}
	}
	if ( bad == 0 ) {
		printf ( "  OK!\n" );
...
void FluidSystem::SetupExampleParams ( bool bStart )
{
	...
	ParseXML ( "Scene", (int) m_Param[PEXAMPLE], bStart );
}
